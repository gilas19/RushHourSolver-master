...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer):
        ...
        for action in all_actions:
            if previous_proposition_layer.all_preconds_in_layer(action):
                has_pairwise_mutex = False
                for prop1, prop2 in combinations(action.get_pre(), 2):
                    if previous_proposition_layer.is_mutex(prop1, prop2):
                        has_pairwise_mutex = True
                        break
                if not has_pairwise_mutex:
    ...
    def update_mutex_actions(self, previous_layer_mutex_proposition):
        ...
        "*** YOUR CODE HERE ***"
        for action1, action2 in combinations(current_layer_actions, 2):
        ...
        for action in current_layer_actions:
            for prop in action.get_add():
                if prop not in propositions:
                    propositions[prop] = Proposition(prop.get_name())
                propositions[prop].add_producer(action)
    ...
    def update_mutex_proposition(self):
        ...
        "*** YOUR CODE HERE ***"
        for prop1, prop2 in combinations(current_layer_propositions, 2):
